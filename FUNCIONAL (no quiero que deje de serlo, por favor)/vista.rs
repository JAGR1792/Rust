...
struct MeshCache {
    ...
    vehiculos: [graphics::Mesh; 5], // Carro, Camioneta, Camión, Ambulancia, Policia
...
pub fn dibujar_vehiculo(canvas: &mut graphics::Canvas, ctx: &mut Context, carro: &Carro) -> GameResult {
    let mesh_idx = match carro.tipo {
        ...
        TipoVehiculo::Ambulancia => 3,
        TipoVehiculo::Policia => 4,
    };
    ...
    if carro.loco {
        ...
        let (ancho, alto) = match carro.tipo {
            ...
            TipoVehiculo::Ambulancia => (40.0,24.0),
            TipoVehiculo::Policia => (35.0,20.0),
        };
    ...
    if let Some(ref guard) = get_cache() {
        if let Some(ref cache) = **guard {
            ...
            match carro.tipo {
                TipoVehiculo::Ambulancia | TipoVehiculo::Policia=> {
                    // Sirena de emergencia
                    let pos_x = carro.posicion[0];
                    let pos_y = carro.posicion[1];

                    // Ajustar posición según rotación
                    let (sirena_x, sirena_y, sirena_ancho, sirena_alto) = if carro.direccion == "este" {
                        (pos_x + 10.0, pos_y + 3.0, 6.0, 4.5)
                    } else {
                        (pos_x + 3.0, pos_y + 8.0, 4.5, 6.0)
                    };

                    let mitad_ancho = sirena_ancho / 2.0;

                    let sirena_azul = graphics::Mesh::new_rectangle(
                        ctx,
                        graphics::DrawMode::fill(),
                        graphics::Rect::new(0.0, 0.0, mitad_ancho, sirena_alto),
                        graphics::Color::new(0.0, 0.0, 1.0, 1.0), // Azul
                    )?;

                    let sirena_roja = graphics::Mesh::new_rectangle(
                        ctx,
                        graphics::DrawMode::fill(),
                        graphics::Rect::new(mitad_ancho, 0.0, mitad_ancho, sirena_alto),
                        graphics::Color::new(1.0, 0.0, 0.0, 1.0), // Rojo
                    )?;

                    canvas.draw(&sirena_azul, graphics::DrawParam::new().dest([sirena_x, sirena_y]).rotation(rotacion));
                    canvas.draw(&sirena_roja, graphics::DrawParam::new().dest([sirena_x, sirena_y]).rotation(rotacion));

                },
                TipoVehiculo::Automovil | TipoVehiculo::Policia=> {
                ...
                TipoVehiculo::Camioneta | TipoVehiculo::Camion | TipoVehiculo::Ambulancia=> {
    ...
    let (ancho, alto) = match carro.tipo {
        ...
        TipoVehiculo::Policia => (32.0, 17.0),
        TipoVehiculo::Ambulancia => (40.0, 20.0),
    };
